...
{
    // 普通に描画されたテクスチャ
    float4 combineColor = textureMaps[0].Sample(samplerStates[_anisotropicSampler], pin.texcoord);
    
    for (int i = 0; i < GAUSSIAN_DOWNSAMPLING_COUNT; i++)
    {
        combineColor += textureMaps[i].Sample(samplerStates[_anisotropicSampler], pin.texcoord);
    }
